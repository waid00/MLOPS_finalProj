...
def apply_windows_torch_patch():
    ...
    if platform.system() == "Windows":
        ...
        try:
            # Přidání lib složky torch do PATH pro načítání závislostí
            spec = find_spec("torch")
            if spec and spec.origin:
                torch_lib_path = Path(spec.origin).parent / "lib"
                if torch_lib_path.exists():
                    # Nový způsob pro Python 3.8+
                    os.add_dll_directory(str(torch_lib_path))
                    # Starší způsob jako fallback / pro jistotu
...
# Inicializace logování před vším ostatním
logging.basicConfig(
    level=logging.INFO,
...
# Importy tvých modulů (ujisti se, že jsou v PYTHONPATH)
try:
...
def run_experiments():
    ...
    """
    # Načtení dat (Business Report: Data Processing Decisions)
    ...
    # Matice experimentů (Requirement: Systematic Exploration)
    experiments = [
        {"umap_neighbors": 15, "min_cluster_size": 5,  "note": "Baseline - High Granularity"},
        {"umap_neighbors": 15, "min_cluster_size": 10, "note": "Target Configuration"},
        {"umap_neighbors": 15, "min_cluster_size": 20, "note": "Strict Trend Detection"},
        {"umap_neighbors": 5,  "min_cluster_size": 10, "note": "Local Structure Focus"},
        {"umap_neighbors": 30, "min_cluster_size": 10, "note": "Global Semantic Focus"}
    ]

    # Použití silného modelu (Requirement: Hyperparameter Justification)
    ...
    for i, params in enumerate(experiments):
        run_name = f"Exp_{i}_N{params['umap_neighbors']}_S{params['min_cluster_size']}"
        
        with mlflow.start_run(run_name=run_name):
            logger.info(f"Starting experiment: {run_name}")
            
            # Parametrizace modelů (Requirement: Parameter Variations)
            umap_model = UMAP(
            ...
            # Clean Code: Vectorizer s odstraněním stop-words
            ...
            # Výpočet metrik pro Business Report
            topic_info = topic_model.get_topic_info()
            n_topics = len(topic_info) - 1 # Bez outlierů
            ...
            # MLflow Logging (Requirement: Documentation of process)
            mlflow.log_params({
            ...
            # Logování artefaktu (Requirement: Saved artifacts)
